...
const LOCK_FILE = join(tmpdir(), "dhf-tencent-news-task.lock");
...
function parseArgs(args: string[]): TencentNewsOptions {
  const options: Partial<TencentNewsOptions> = {};
  ...
  return options as TencentNewsOptions;
}

// Show help
function showHelp() {
  printHeader("DHF Tencent News Task - 腾讯新闻热门");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /dhf-tencent-news-task [options]", colors.cyan);
  console.log();
  ...
  console.log();
  log("  此任务获取腾讯新闻的热门资讯", colors.gray);
  console.log();
  log("示例:", colors.bright);
  console.log();
  log("  # 获取热门新闻", colors.gray);
  log("  /dhf-tencent-news-task", colors.green);
  console.log();
  log("  # 限制返回 10 条", colors.gray);
  log("  /dhf-tencent-news-task --limit 10", colors.green);
  console.log();
  log("  # 保存结果", colors.gray);
  log("  /dhf-tencent-news-task --output \"./news.json\"", colors.green);
  console.log();
  log("任务信息:", colors.bright);
  console.log();
  log(`  任务 ID: ${TASK_ID}`, colors.cyan);
  log(`  MCP 服务: dhf_rpa_task`, colors.cyan);
  log(`  访问网站: 腾讯新闻`, colors.cyan);
  console.log();
}
...
function displayNewsResults(news: TencentNewsResult[], title: string): void {
  console.log();
  log("═══════════════════════════════════════════════════════════════════", colors.cyan);
  log(`  ${title}`, colors.bright + colors.cyan);
  ...
  news.forEach((item, index) => {
    ...
    if (item.source) {
      log(`   来源: ${item.source} ${item.publish_time ? '| ' + item.publish_time : ''}`, colors.gray);
...
function saveResultsToFile(news: TencentNewsResult[], title: string, filePath: string): void {
  const output = {
    title,
...
// Get news
async function getNews(options: TencentNewsOptions, verbose = false): Promise<void> {
  const startTime = Date.now();
  printHeader("腾讯新闻热门");
  ...
  log("🚀 启动腾讯新闻任务...", colors.yellow);
  log(`   任务 ID: ${TASK_ID}`, colors.cyan);
  log(`   目标: 腾讯新闻`, colors.cyan);
  console.log();
  ...
  if (execResult.success && execResult.result) {
    ...
    if (news.length > 0) {
      ...
      displayNewsResults(displayNews, "腾讯新闻热门");

      // Save to file if output path is specified
      if (options.output) {
        saveResultsToFile(displayNews, "腾讯新闻热门", options.output);
      }
    } else {
      log("⚠️ 未获取到新闻", colors.yellow);
      log("   建议:", colors.gray);
      log("   - 检查网络连接", colors.gray);
      log("   - 确认腾讯新闻网站可访问", colors.gray);
    }
