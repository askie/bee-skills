...
const TASK_ID = "aTGAEx";
const OUTPUT_FILE = "hot_topics.json";
const MCP_ENDPOINT = "/mcp";  // 正确的 MCP 端点
...
interface GoogleHotTopicResult {
  rank?: number;
  topic: string;
...
interface GoogleHotTopicsResponse {
  success: boolean;
  data: GoogleHotTopicResult[];
  total: number;
  country: string;
  source: string;
}
 ...
 * 从本地文件读取任务运行结果
 */
function getLocalRunResult(runId: string): GoogleHotTopicResult[] | null {
  const dataDir = getDHFDataDir();
  const runDir = join(dataDir, TASK_ID, "run");

  log(`\n📂 尝试从本地读取结果...`, colors.cyan);

  // 方法1: 直接读取 hot_topics.json 文件
  const topicsPath = join(runDir, OUTPUT_FILE);
  if (existsSync(topicsPath)) {
    try {
      const content = readFileSync(topicsPath, "utf-8");
      const data = JSON.parse(content);
      if (Array.isArray(data) && data.length > 0) {
        log(`✅ 成功从 ${OUTPUT_FILE} 读取到 ${data.length} 条话题`, colors.green);
        return data;
      }
    } catch (e) {
  ...
  // 方法2: 尝试其他可能的文件名
  const possibleFiles = ["result.json", "trends.json", "google_hot.json"];
    ...
    if (existsSync(resultPath)) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          log(`✅ 成功从 ${fileName} 读取到 ${parsed.length} 条话题`, colors.green);
          return parsed;
        }
      } catch {
        continue;
    ...
    if (finishFiles.length > 0) {
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            log(`✅ 成功从 finish.md 读取到 ${data.length} 条话题`, colors.green);
            return data;
          }
        }
      } catch (e) {
    ...
    switch (arg) {
      ...
      default:
        if (arg.startsWith("--")) {
          logError(`未知参数: ${arg}`);
          console.log("\n使用方法:");
          console.log("  /dhf-google-hot-topics-task [选项]");
          console.log("\n选项:");
          console.log("  --country, -c <代码>   国家/地区代码 (US, CN, GB, JP...)");
          console.log("  --limit, -n <数量>     限制返回的话题数量");
...
async function startTask(options: Options): Promise<string> {
  ...
  log(`\n🚀 启动 Google 热门话题任务 (ID: ${TASK_ID})...`, colors.cyan);
  log(`   📍 地区: ${country}`, colors.gray);
 ...
 * 轮询任务执行结果
 */
async function pollExecution(runId: string, options: Options): Promise<GoogleHotTopicResult[]> {
  ...
  while (Date.now() - startTime < MAX_POLL_TIME) {
    try {
      ...
      if (status === "success") {
        ...
        // 尝试从 result 字段中获取话题数据
        let topics: GoogleHotTopicResult[] | undefined;
        ...
        if (result.result) {
          try {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
              topics = parsed;
            } else if (parsed.topics || parsed.trending) {
              topics = parsed.topics || parsed.trending;
            }
          } catch {
            // 继续尝试其他方式
          }
        ...
        if (topics && topics.length > 0) {
          log(`📦 从响应获取到 ${topics.length} 条话题`, colors.green);
          return topics;
        } else {
          logWarning(`⚠️  响应未包含话题数据，尝试从本地文件读取...`);
 ...
 * 显示话题结果
 */
function displayResults(topics: GoogleHotTopicResult[], options: Options): void {
  const country = options.country || "US";
  const limit = options.limit || topics.length;
  const displayList = topics.slice(0, limit);

  console.log(`\n${colors.bright}${colors.cyan}═══════════════════════════════════════════════════════${colors.reset}`);
  console.log(`${colors.bright}${colors.cyan}      Google 热门话题 (${country})${colors.reset}`);
  console.log(`${colors.bright}${colors.cyan}═══════════════════════════════════════════════════════${colors.reset}\n`);

  if (displayList.length === 0) {
    logError("❌ 没有找到话题数据");
    logWarning(`💡 提示: Google Trends 在中国大陆可能需要特殊网络环境`);
    return;
  }
  ...
  displayList.forEach((item, index) => {
    ...
    // 话题名称
    console.log(`${rankDisplay}${colors.bright}${colors.white}${item.topic}${colors.reset}`);

    // 搜索量和相关查询（verbose 模式）
    if (options.verbose) {
      if (item.search_volume) {
        console.log(`    ${colors.cyan}搜索量: ${item.search_volume}${colors.reset}`);
      }
      if (item.related_queries && item.related_queries.length > 0) {
        console.log(`    ${colors.gray}相关: ${item.related_queries.slice(0, 3).join(', ')}${colors.reset}`);
      }
    }
  ...
  console.log(`${colors.gray}共获取 ${topics.length} 条话题，显示前 ${limit} 条${colors.reset}\n`);
}

/**
 * 保存结果到文件
 */
function saveResults(topics: GoogleHotTopicResult[], outputPath: string, country: string): void {
  try {
    ...
    const response: GoogleHotTopicsResponse = {
      success: true,
      data: topics,
      total: topics.length,
      country: country,
...
async function main() {
  ...
  console.log(`\n${colors.bright}${colors.cyan}╔═══════════════════════════════════════════════════════╗${colors.reset}`);
  console.log(`${colors.bright}${colors.cyan}║    DHF Google 热门话题任务 v1.0.0                ║${colors.reset}`);
  console.log(`${colors.bright}${colors.cyan}╚═══════════════════════════════════════════════════════╝${colors.reset}`);
  ...
  try {
    ...
    // 轮询结果
    let topics = await pollExecution(runId, options);

    // 过滤无效数据
    topics = topics.filter((item: any) =>
      item && (item.topic || item.title)
    );

    // 显示结果
    displayResults(topics, options);

    // 保存结果
    if (options.output) {
      saveResults(topics, options.output, options.country || "US");
    }
