...
// Types
interface DouyinHotSearchOptions {
  limit?: number;
  output?: string;
  check?: boolean;
  verbose?: boolean;
...
interface DouyinHotSearchResult {
  title: string;
  heat?: string;
  link: string;
}
...
const LOCK_FILE = join(tmpdir(), "dhf-douyin-hot-search-task.lock");
...
function parseArgs(args: string[]): DouyinHotSearchOptions {
  const options: Partial<DouyinHotSearchOptions> = {};
  ...
  return options as DouyinHotSearchOptions;
}

// Show help
function showHelp() {
  printHeader("DHF Douyin Hot Search Task - 抖音热搜榜单");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /dhf-douyin-hot-search-task [options]", colors.cyan);
  console.log();
  ...
  console.log();
  log("  此任务获取抖音热搜的实时榜单", colors.gray);
  log("  🎵 包含热搜词、热度值、链接等信息", colors.gray);
  console.log();
  log("示例:", colors.bright);
  console.log();
  log("  # 获取热搜榜", colors.gray);
  log("  /dhf-douyin-hot-search-task", colors.green);
  console.log();
  log("  # 获取前20名", colors.gray);
  log("  /dhf-douyin-hot-search-task --limit 20", colors.green);
  console.log();
  log("  # 保存结果", colors.gray);
  log("  /dhf-douyin-hot-search-task --output \"./douyin-hot.json\"", colors.green);
  console.log();
  log("任务信息:", colors.bright);
  console.log();
  log(`  任务 ID: ${TASK_ID}`, colors.cyan);
  log(`  MCP 服务: dhf_rpa_task`, colors.cyan);
  log(`  访问网站: 抖音`, colors.cyan);
  console.log();
}
...
function getLocalRunResult(taskId: string, runId: string): { success: boolean; status?: string; error?: string; result?: any } | null {
  ...
  try {
    ...
    if (status === "success" || status === "completed" || status === "SUCCESS") {
      ...
      const possibleFiles = ["hotList.json", "hot_search.json", "result.json"];
...
function displayHotSearchResults(items: DouyinHotSearchResult[], title: string): void {
  console.log();
  log("═══════════════════════════════════════════════════════════════════", colors.cyan);
  log(`  ${title}`, colors.bright + colors.cyan);
  ...
  items.forEach((item, index) => {
    ...
    if (item.heat) {
      log(`   🔥 热度: ${item.heat}`, colors.magenta);
    }

    if (item.link) {
      // Convert link to full URL if it's a relative path
      const fullUrl = item.link.startsWith('http') ? item.link : `https://www.douyin.com${item.link}`;
      log(`   🔗 链接: ${fullUrl}`, colors.blue);
    }
...
function saveResultsToFile(items: DouyinHotSearchResult[], title: string, filePath: string): void {
  const output = {
    title,
...
// Get hot search
async function getHotSearch(options: DouyinHotSearchOptions, verbose = false): Promise<void> {
  const startTime = Date.now();
  printHeader("抖音热搜榜单");
  ...
  log("🚀 启动抖音热搜任务...", colors.yellow);
  log(`   任务 ID: ${TASK_ID}`, colors.cyan);
  log(`   目标: 抖音热搜`, colors.cyan);
  console.log();
  ...
  if (execResult.success && execResult.result) {
    ...
    let items: DouyinHotSearchResult[] = [];
    ...
    if (items.length > 0) {
      ...
      displayHotSearchResults(displayItems, "抖音热搜榜单");

      // Save to file if output path is specified
      if (options.output) {
        saveResultsToFile(displayItems, "抖音热搜榜单", options.output);
      }
    } else {
      log("⚠️ 未获取到热搜数据", colors.yellow);
      log("   可能的原因:", colors.gray);
      log("   - 抖音页面需要登录", colors.gray);
      log("   - 网络连接问题", colors.gray);
      log("   - 页面结构变化", colors.gray);
    }
