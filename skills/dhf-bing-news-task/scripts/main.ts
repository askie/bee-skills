...
const LOCK_FILE = join(tmpdir(), "dhf-bing-news-task.lock");
...
function parseArgs(args: string[]): BingNewsOptions {
  const options: Partial<BingNewsOptions> = {};
  ...
  return options as BingNewsOptions;
}

// Show help
function showHelp() {
  printHeader("DHF Bing News Task - 必应新闻热门");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /dhf-bing-news-task [options]", colors.cyan);
  console.log();
  ...
  console.log();
  log("  此任务获取必应新闻的热门资讯", colors.gray);
  log("  ⚠️ 注意：必应新闻在中国大陆可能需要特殊网络环境", colors.yellow);
  console.log();
  log("示例:", colors.bright);
  console.log();
  log("  # 获取热门新闻", colors.gray);
  log("  /dhf-bing-news-task", colors.green);
  console.log();
  log("  # 限制返回 10 条", colors.gray);
  log("  /dhf-bing-news-task --limit 10", colors.green);
  console.log();
  log("  # 保存结果", colors.gray);
  log("  /dhf-bing-news-task --output \"./news.json\"", colors.green);
  console.log();
  log("任务信息:", colors.bright);
  console.log();
  log(`  任务 ID: ${TASK_ID}`, colors.cyan);
  log(`  MCP 服务: dhf_rpa_task`, colors.cyan);
  log(`  访问网站: 必应新闻`, colors.cyan);
  console.log();
}
...
function displayNewsResults(news: BingNewsResult[], title: string): void {
  console.log();
  log("═══════════════════════════════════════════════════════════════════", colors.cyan);
  log(`  ${title}`, colors.bright + colors.cyan);
...
function saveResultsToFile(news: BingNewsResult[], title: string, filePath: string): void {
  const output = {
    title,
...
// Get news
async function getNews(options: BingNewsOptions, verbose = false): Promise<void> {
  const startTime = Date.now();
  printHeader("必应新闻热门");
  ...
  log("🚀 启动必应新闻任务...", colors.yellow);
  log(`   任务 ID: ${TASK_ID}`, colors.cyan);
  log(`   目标: 必应新闻`, colors.cyan);
  console.log();
  ...
  if (execResult.success && execResult.result) {
    ...
    if (news.length > 0) {
      ...
      displayNewsResults(displayNews, "必应新闻热门");

      // Save to file if output path is specified
      if (options.output) {
        saveResultsToFile(displayNews, "必应新闻热门", options.output);
      }
    } else {
      log("⚠️ 未获取到新闻", colors.yellow);
      log("   可能的原因:", colors.gray);
      log("   - 必应新闻在当前地区可能无法访问", colors.gray);
      log("   - 网络连接问题", colors.gray);
      log("   - 建议使用其他新闻源（腾讯新闻、网易新闻）", colors.yellow);
    }
    ...
    log("  - 必应新闻页面结构可能已更新", colors.gray);
    log("  - 网络连接问题", colors.gray);
    log("  - 地区访问限制", colors.gray);
    process.exit(1);
  }
}
