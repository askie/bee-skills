...
自动调用 DHF Agent **任务**，通过浏览器获取必应（Bing）新闻资讯。
...
- ✅ 自动打开浏览器
- ✅ 访问必应新闻 (https://www.bing.com/news)
- ✅ 获取热门新闻资讯
- ✅ 提取新闻标题、链接、时间、来源
...
# 显示详细输出
/dhf-bing-news-task --verbose

# 检查 DHF Agent 连接
/dhf-bing-news-task --check
```
...
```bash
/dhf-bing-news-task --output "./bing-news.json"
```
...
**解决：**
- 检查网络连接
- 确保可以访问 www.bing.com/news
- **注意**：在中国大陆可能需要特殊网络环境访问 Bing
...
- **必应新闻：** https://www.bing.com/news

## 特别说明

⚠️ **网络限制提示**

必应（Bing）新闻服务在中国大陆地区可能受到访问限制。如果遇到无法访问或返回空结果的情况，请：

1. 检查网络连接
2. 尝试使用可访问 Bing 的网络环境
3. 或使用其他新闻源（如腾讯新闻、网易新闻）
