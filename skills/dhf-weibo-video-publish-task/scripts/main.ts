#!/usr/bin/env bun
/**
 * DHF Weibo Video Publish Task
 *
 * 自动调用 DHF Agent 任务发布视频到微博平台
 */
...
function showHelp() {
  printHeader("微博视频发布任务 - Weibo Video Publish Task");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /dhf-weibo-video-publish-task [选项]", colors.cyan);
  console.log();
  ...
  console.log();
  log("  /dhf-weibo-video-publish-task -c \"hello！\" -v \"./video.mp4\"", colors.green);
  log("  /dhf-weibo-video-publish-task -c \"内容\" -v \"./v1.mp4\" -v \"./v2.mp4\"", colors.green);
  log("  /dhf-weibo-video-publish-task -c \"内容\" -v \"./video.mp4\" -a \"N\"", colors.green);
  console.log();
...
async function startPublishTask(input: TaskInput): Promise<string> {
  log(`\n🚀 启动微博视频发布任务 (ID: ${TASK_ID})...`, colors.cyan);
  log(`   📝 内容: ${input.initialState.content}`, colors.gray);
