---
name: dhf-xiaohongshu-video-publish-task
description: 调用 DHF Agent 任务自动发布视频笔记到小红书平台。支持标题、内容、视频路径配置
version: 1.0.0
metadata:
  tags: [dhf, xiaohongshu, xhs, publish, video, automation]
...
自动调用 DHF Agent **任务**，发布视频笔记到小红书平台。
...
```bash
# 基本发布
/dhf-xiaohongshu-video-publish-task --title "标题" --content "内容" --video "./video.mp4"

# 多个视频
/dhf-xiaohongshu-video-publish-task --title "标题" --content "内容" --video "./v1.mp4" --video "./v2.mp4"

# 保存草稿
/dhf-xiaohongshu-video-publish-task --title "标题" --content "内容" --video "./video.mp4" --autoSend "N"

# 检查 DHF Agent 连接
/dhf-xiaohongshu-video-publish-task --check
```
