...
function showHelp() {
  printHeader("小红书视频发布任务 - Xiaohongshu Video Publish Task");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /dhf-xiaohongshu-video-publish-task [选项]", colors.cyan);
  console.log();
  ...
  console.log();
  log("  /dhf-xiaohongshu-video-publish-task -t \"标题\" -c \"内容\" -v \"./video.mp4\"", colors.green);
  log("  /dhf-xiaohongshu-video-publish-task -t \"标题\" -c \"内容\" -v \"./v1.mp4\" -v \"./v2.mp4\"", colors.green);
  log("  /dhf-xiaohongshu-video-publish-task -t \"标题\" -c \"内容\" -v \"./video.mp4\" -a \"N\"", colors.green);
  console.log();
...
async function startPublishTask(input: TaskInput): Promise<string> {
  log(`\n🚀 启动小红书视频发布任务 (ID: ${TASK_ID})...`, colors.cyan);
  ...
  try {
    ...
    if (execResult.success) {
      ...
      if (options.autoSend === "Y") {
        ...
        log(`   提示: 请到小红书后台查看并手动发布`, colors.gray);
      }
      console.log();
    } else {
