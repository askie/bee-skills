#!/usr/bin/env bun
/**
 * DHF Zhihu Video Publish Task
 *
 * 自动调用 DHF Agent 任务发布视频笔记到知乎平台
 */
...
function showHelp() {
  printHeader("知乎视频发布任务 - Zhihu Video Publish Task");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /dhf-zhihu-video-publish-task [选项]", colors.cyan);
  console.log();
  log("必填参数:", colors.bright);
  console.log();
  log("  --title, -t       视频标题", colors.cyan);
  log("  --content, -c     视频描述内容", colors.cyan);
  log("  --video, -v       视频文件路径（可多次使用）", colors.cyan);
  console.log();
  log("可选参数:", colors.bright);
  console.log();
  log("  --autoSend, -a    自动发布 Y/N (默认: N)", colors.cyan);
  log("  --check           检查 DHF Agent 连接", colors.cyan);
  log("  --help, -h        显示帮助", colors.cyan);
  console.log();
  log("示例:", colors.bright);
  console.log();
  log("  /dhf-zhihu-video-publish-task -t \"标题\" -c \"内容\" -v \"./video.mp4\"", colors.green);
  log("  /dhf-zhihu-video-publish-task -t \"标题\" -c \"内容\" -v \"./v1.mp4\" -v \"./v2.mp4\"", colors.green);
  log("  /dhf-zhihu-video-publish-task -t \"标题\" -c \"内容\" -v \"./video.mp4\" -a \"Y\"", colors.green);
  console.log();
...
async function startPublishTask(input: TaskInput): Promise<string> {
  log(`\n🚀 启动知乎视频发布任务 (ID: ${TASK_ID})...`, colors.cyan);
  ...
  try {
    ...
    if (execResult.success) {
      ...
      if (options.autoSend === "Y") {
        ...
        log(`   提示: 请到知乎后台查看并手动发布`, colors.gray);
      }
      console.log();
    } else {
