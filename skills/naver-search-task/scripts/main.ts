...
function showHelp() {
  printHeader("Naver 搜索任务 - Naver Search Task");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /naver-search-task [选项]", colors.cyan);
  console.log();
  ...
  console.log();
  log("  /naver-search-task -k \"大黄蜂科技\"", colors.green);
  log("  /naver-search-task -k \"大黄蜂科技\" -p 3", colors.green);
  console.log();
...
async function search(options: SearchOptions): Promise<void> {
  ...
  log("🚀 启动 Naver 搜索任务...", colors.yellow);
  log(`   任务 ID: ${TASK_ID}`, colors.cyan);
  console.log();
