...
description: 调用 DHF Agent 任务进行 Naver 搜索。输入关键词，浏览器自动打开执行搜索
version: 1.0.0
metadata:
  tags: [dhf, automation, search, naver, task]
...
自动调用 DHF Agent **任务**，在 Naver 搜索中查找关键词。
...
```bash
# 基本调用
/naver-search-task -k "大黄蜂科技"

# 指定搜索页数
/naver-search-task -k "大黄蜂科技" -p 3

# 检查 DHF Agent 连接
/naver-search-task --check
```
