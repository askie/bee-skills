...
function showHelp() {
  printHeader("今日头条视频发布任务 - Toutiao Video Publish Task");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /dhf-toutiao-video-publish-task [选项]", colors.cyan);
  console.log();
  ...
  console.log();
  log("  /dhf-toutiao-video-publish-task -t \"标题\" -c \"内容\" -v \"./video.mp4\"", colors.green);
  log("  /dhf-toutiao-video-publish-task -t \"标题\" -c \"内容\" -v \"./v1.mp4\" -v \"./v2.mp4\"", colors.green);
  log("  /dhf-toutiao-video-publish-task -t \"标题\" -c \"内容\" -v \"./video.mp4\" -a \"N\"", colors.green);
  console.log();
...
async function startPublishTask(input: TaskInput): Promise<string> {
  log(`\n🚀 启动今日头条视频发布任务 (ID: ${TASK_ID})...`, colors.cyan);
  ...
  try {
    ...
    if (execResult.success) {
      ...
      if (options.autoSend === "Y") {
        ...
        log(`   提示: 请到今日头条后台查看并手动发布`, colors.gray);
      }
      console.log();
    } else {
